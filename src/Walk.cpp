...
string Skill::SideWalkLib(int &t, SideT side) {
    ...
    KDL::Frame lfrm(KDL::Vector(lAnkleStanding.p.x() + x1, lAnkleStanding.p.y(), lAnkleStanding.p.z() + y1));

    rLegFKSolver->JntToCart(jointpositionsr, rHipStanding);
    KDL::Frame rfrm(KDL::Vector(rHipStanding.p.x() + x2, rHipStanding.p.y(), rHipStanding.p.z() + y2));
...
string Skill::WalkAngleLib(int &t, SideT side) {
    ...
    double x2 = -a1 * cos(w * t * 0.02 + M_PI) * sin(theta), y2 = a1 * cos(w * t * 0.02 + M_PI) * cos(theta), z2 = b1 * sin(w * t * 0.02 + M_PI);
...
string Skill::TurnLib(int &t, SideT side) {
    ...
    KDL::Frame lfrm(lLegRot, KDL::Vector(lAnkleStanding.p.x(), lAnkleStanding.p.y(), lAnkleStanding.p.z() + z1));
    KDL::Frame rfrm(rHipRot, KDL::Vector(rHipStanding.p.x(), rHipStanding.p.y(), rHipStanding.p.z() + z2));
...
string Skill::moveJoints(KDL::JntArray left, KDL::JntArray right, double p) {
    ...
    TT = Tint * 0.02;
    ww = (2 * M_PI) / TT;
    sc = sc + moveJoint("lae1", B1 + A1 * sin(ww * ta + fi) - WM->getJointAngle("lae1"))
