...
{
    fr=false;fn=false;fs=false;pr=false;prn=false;prs=false;m=false,fl=false,pl=false,pln=false,pls=false;
    frfn=false;frfs=false;frprn=false;frprs=false;prprn=false;prprs=false;ms=false;mn=false,
...
void Localize::setX()
{
    ...
    if(frprs && prprs)
    {
        ...
        name2="prprs";
        base = 1.8;
        len1 = distance_of_important_points["FRPRS"];len2 = distance_of_important_points["PRPRS"] ;
    ...
    if(flpls && plpls)
    {
        ...
        name2="plpls";
        base = 1.8;
        len1 = distance_of_important_points["FLPLS"];len2 = distance_of_important_points["PLPLS"] ;
        s=Area(len1 ,len2 ,base);
        h=2*s/base;
        x1=sqrt(len1*len1 - h*h);
        x2=sqrt(len2*len2 - h*h);
    ...
    if(frprn && prprn)
    {
        ...
        name2="prprn";
        base = 1.8;
        len1 = distance_of_important_points["FRPRN"];len2 = distance_of_important_points["PRPRN"] ;
        s=Area(len1 ,len2 ,base);
        h=2*s/base;
        x1=sqrt(len1*len1 - h*h);
        x2=sqrt(len2*len2 - h*h);
    ...
    if(flpln && plpln)
    {
        ...
        name2="plpln";
        base = 1.8;
        len1 = distance_of_important_points["FLPLN"];len2 = distance_of_important_points["PLPLN"] ;
        s=Area(len1 ,len2 ,base);
        h=2*s/base;
        x1=sqrt(len1*len1 - h*h);
        x2=sqrt(len2*len2 - h*h);
    ...
    if(ms && frfs)
    {
        ...
        len1 = distance_of_important_points["FRFS"];len2 = distance_of_important_points["MS"] ;
        s=Area(len1 ,len2 ,base);
        h=2*s/base;
        x1=sqrt(len1*len1 - h*h);
        x2=sqrt(len2*len2 - h*h);
    ...
    if(ms && flfs)
    {
        ...
        len1 = distance_of_important_points["FLFS"];len2 = distance_of_important_points["MS"] ;
        s=Area(len1 ,len2 ,base);
        h=2*s/base;
        x1=sqrt(len1*len1 - h*h);
        x2=sqrt(len2*len2 - h*h);
    ...
    if(mn && frfn)
    {
        ...
        name2="frfn";
        base = 15;
        len1 = distance_of_important_points["FRFN"];len2 = distance_of_important_points["MN"] ;
        s=Area(len1 ,len2 ,base);
        h=2*s/base;
        x1=sqrt(len1*len1 - h*h);
        x2=sqrt(len2*len2 - h*h);
    ...
    if(mn && flfn)
    {
        ...
        name2="flfn";
        base = 15;
        len1 = distance_of_important_points["FLFN"];len2 = distance_of_important_points["MN"] ;
        s=Area(len1 ,len2 ,base);
        h=2*s/base;
        x1=sqrt(len1*len1 - h*h);
        x2=sqrt(len2*len2 - h*h);
...
void Localize::setY()
{
    ...
    if((frfn && frfs) || (flfn && flfs))
    {
        if(frfn && frfs)
        {
            name1="frfn";
            name2="frfs";
            len1 = distance_of_important_points["FRFN"];len2 = distance_of_important_points["FRFS"] ;
        }
        else{
            name1="flfn";
            name2="flfs";
            len1 = distance_of_important_points["FLFN"];len2 = distance_of_important_points["FLFS"] ;
        }
    ...
    if((frfn && frprs) || (flfn && flpls))
    {
        ...
        if(frfn && frprs){
            name1="frfn";
            name2="frprs";
            len1 = distance_of_important_points["FRFN"];len2 = distance_of_important_points["FRPRS"] ;
        }
        else{
            name1="flfn";
            name2="flpls";
            len1 = distance_of_important_points["FLFN"];len2 = distance_of_important_points["FLPLS"] ;
        }
    ...
    if((frprn && frfs) || (flpln && flfs))
    {
        if((frprn && frfs) ){
            name1="frprn";
            name2="frfs";
            len1 = distance_of_important_points["FRPRN"];len2 = distance_of_important_points["FRFS"] ;

        }
        else{
            name2="flpln";
            name2="flfs";
            len1 = distance_of_important_points["FLPLN"];len2 = distance_of_important_points["FLFS"] ;
    ...
    if((frprs && frfs) || (flpls && flfs))
    {
        if((frprs && frfs) ){
            name1="frprs";
            name2="frfs";
            len1 = distance_of_important_points["FRPRS"];len2 = distance_of_important_points["FRFS"] ;
        }
        else{
            name1="flpls";
            name2="flfs";
            len1 = distance_of_important_points["FLPLS"];len2 = distance_of_important_points["FLFS"] ;
        }
    ...
    if((frfn && frprn) ||(flfn && flpln))
    {
        if((frfn && frprn) ){
            name1="frfn";
            name2="frprn";
            len1 = distance_of_important_points["FRFN"];len2 = distance_of_important_points["FRPRN"] ;
        }
        else{
            name1="flfn";
            name2="flpln";
            len1 = distance_of_important_points["FLFN"];len2 = distance_of_important_points["FLPLN"] ;
        }
    ...
    if((frprn && frprs) || (flpln && flpls))
    {
        if(frprn && frprs) {
            name1="frprn";
            name2="frprs";
            len1 = distance_of_important_points["FRPRN"];len2 = distance_of_important_points["FRPRS"] ;

        }
        else{
            name1="flpln";
            name2="flpls";
            len1 = distance_of_important_points["FLPLN"];len2 = distance_of_important_points["FLPLS"] ;
        }
    ...
    if((prprn && prprs) || (plpln && plpls))
    {
        if((prprn && prprs) ){
            name1="prprs";
            name2="prprn";
            len1 = distance_of_important_points["PRPRN"];len2 = distance_of_important_points["PRPRS"] ;
        }
        else{
            name1="plpls";
            name2="plpln";
            len1 = distance_of_important_points["PLPLN"];len2 = distance_of_important_points["PLPLS"] ;
...
void Localize::set_second_x_estimation()
{
    ...
    for(unordered_map<string,Vector3f>::iterator f=importan_points.begin();f!=importan_points.end();f++)
    {
        ...
        if(length!=0 && !(f->first == "MS" || f->first == "MN")/*checks that we realy see a good flag*/){
            ...
            if(fabs((fl.x() - diff) - X_initial_estimation) < fabs(fl.x() + diff -X_initial_estimation) &&
                    (fl.x()-diff < 16 && fl.x() - diff > -16))
            {
                x=fl.x() - diff;
                sum+=fl.x() - diff;
                counter++;
...
void Localize::set_head_rotate(Vector3f position)
{
    ...
    Eigen::Matrix3f orient = I_P_glabal*(I_P_local.transpose())*((I_P_local*I_P_local.transpose()).inverse());
    salt::Matrix myOr(orient(0,0),orient(0,1),orient(0,2),0
...
Vector3f Localize::calcPlaneNormal(vector<Vector3f> planePoints)
{
    ...
    Eigen::SVD<Eigen::MatrixXf>  svd(covm);
