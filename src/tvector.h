...
public:
  ...
  f_inline TYPE LinearInterpolate(const TYPE& to, double delta) const;

  /** lineary interpolates between this vector and to with an delta
   * increment, returning a normalized vector
   */
  f_inline TYPE NormalizedLinearInterpolate(const TYPE& to, double delta) const
  {
...
public:
  ...
  TVector2(DATATYPE x, DATATYPE y , vType t)  : TVector<DATATYPE, 2, TYPE>()
  {
    if (t== CARTESIAN)
      {
        Set(x, y);
      ...
      {
        double x1,y1;
        x1 = x * gCos(gDegToRad(y));
        y1 = x * gSin(gDegToRad(y));
        Set(x1,y1);
      }
  }
...
public:
  ...
  TVector3(const DATATYPE& x, const DATATYPE& y, const DATATYPE& z , vType t)
    : TVector<DATATYPE, 3, TYPE>()
  {
    if (t== CARTESIAN)
      ...
      {
        double x1,y1,z1;
        x1 = x * gCos(gDegToRad(z)) * gCos(gDegToRad(y));
        y1 = x * gCos(gDegToRad(z)) * gSin(gDegToRad(y));
        z1 = x * gSin(gDegToRad(z));
        Set(x1,y1,z1);
      }
...
template <typename DATATYPE, int ELEMENTS, typename TYPE>
f_inline TYPE TVector<DATATYPE, ELEMENTS, TYPE>::LinearInterpolate(const TYPE& to, double t) const
{
  double it = 1.0f - t;
  TYPE r;
  for (int c=0; c<ELEMENTS; c++)
